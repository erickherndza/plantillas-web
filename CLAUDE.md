...
### 2026-05-25
- Se reemplazó `admin_scraper_crear_url()` en [admin/app.py](admin/app.py) para usar `blueprint` y `componentes` y generar `defaults_json` con helpers `_blueprint_to_layout()` y `_blueprint_to_defaults()`.
- Se actualizó [admin/templates/admin/scraper_plantillas.html](admin/templates/admin/scraper_plantillas.html) para enviar `blueprint` y `componentes` en el payload.
- Se agregó detección real del blueprint en el frontend: secciones del DOM, toggles de componentes y selector de tipo de hero.
- Se refinó la detección del blueprint para aprovechar señales de `data-*` y `aria-label` además de clases e ids.
- Verificación estática: [admin/app.py](admin/app.py) y [admin/templates/admin/scraper_plantillas.html](admin/templates/admin/scraper_plantillas.html) sin errores.
- La validación de importación del módulo quedó pendiente porque el proveedor de terminal devolvió `ENOPRO` al reintentar la ejecución.

### 2026-05-25 (ajuste de creación de sitios)
- Se agregó el merge de `defaults_json` dentro del flujo de creación de sitios en [admin/app.py](admin/app.py), para que los sitios creados desde scraper/wizard hereden el hero, colores, textos y flags personalizados.
- Verificación funcional: reproducción con test client mostró hero_titulo = Repro Scraper, hero_subtitulo = Soluciones profesionales para tu negocio., color_primario = #ff0000, color_footer_bg = #0000ff, color_navbar_bg = #ff0000, menu_servicios = Servicios y comp_whatsapp = 1.
